...
int AddRgaDlg::check_setting_conflict(QString tag, QString ip) {
    ...
    foreach (QString _rga, sl_rgas) {
        if(_rga == tag) {
            QMessageBox::StandardButton btn = QMessageBox::question(this, "Error", "dDuplicate RGA tag.\nOverwrite?", QMessageBox::Yes | QMessageBox::No);
...
void AddRgaDlg::on_btn_save_clicked() {
    ...
    if(!DataHelper::check_rga_conn(rga_ip, rga_port.toUInt())) {
        QMessageBox::warning(nullptr, "Connection error", "Unable to establish connection \n" + s_rgaAddr);
        return;
    }
    QString s_tag = ui->le_rga_tag->text();
    if(s_tag.length() < 1) {
        QMessageBox::warning(nullptr, "Error", "Please enter a unique RGA tag");
        return;
    }
    ...
    if(DataHelper::save_config(qm_rga, s_tag + ".ini", rga_config_path, "Rga")) {
        QMessageBox::information(nullptr, "Success", "The RGA connection is saved");
    } else {
        QMessageBox::warning(nullptr, "Failed", "Please check the Settings");
    }
    if(is_from_edit) {
        this->close();
    }
...
void AddRgaDlg::on_tbl_rgas_cellClicked(int row, int) {
    ...
    if(!qm_values.count()) {
        QMessageBox::warning(nullptr, "Read failure", "No data was read");
        return;
    }
    //read tag
    if(!qm_values.contains("Tag")) {
